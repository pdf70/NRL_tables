...
setwd(output_path) 
load(file = "nrl_ladders_raw.Rdata")   # list - "tables"
load(file="nrl_ladders.Rdata")         # data frame 
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Functions 
make_graph_nrl = function(team_abbrev) {
  data_for_graph = nrl_ladders %>% 
...
for (j in which(wiki_table_format == 1)) {         # exception - seasons with byes in points table
...
nrl_ladders_adj_names = tables_all %>% 
...
nrl_ladders_name_join_fails = nrl_ladders_adj_names %>%
  filter(is.na(abbrev))
...
table_other_adj = table_other %>% 
  ...
  mutate(Season = as.numeric(Season), 
         season_no = seq(1, length(seasons), by = 1), 
         super_league_ind, 
         no_teams_finals)
...
nrl_ladders_all = nrl_ladders_adj_names %>% 
  ...
  mutate(premiers = ifelse(team_full_name == premiers_full_name, 1, 0),
         ...
         gf_years = ifelse(season %in% c(1908, 1924), 0, 1),
...
nrl_all_time_ladder = group_by(nrl_ladders, team_full_name) %>%
  summarise(count = n(),
            Total_Pld = sum(Pld),
            Total_W = sum(W),
            Total_D = sum(D),
...
premiers = filter(nrl_ladders, premiers == 1)
premiers_by_Pos = group_by(premiers, Pos) %>%
  summarise(count = n())

# totals by season
season_totals = group_by(nrl_ladders, season, yr_end) %>%
  summarise(count = n(),
            Total_Pld = sum(Pld),
            Total_W = sum(W),
            Total_D = sum(D),
...
title_race_totals = group_by(nrl_ladders, season, yr_end) %>%
  summarise(count = n(),
...
# totals by club
club_records = group_by(nrl_ladders, current_name) %>%
...
team_streaks = group_by(nrl_ladders, current_name) %>%
  summarise(count = n(),
...
most_winning_margin_season = title_race_totals %>%
  ...
  left_join(nrl_ladders, by = c("season" = "season")) %>%
...
error_check_pd = nrl_ladders %>%
  filter(!(PD_check == 0))

error_check_pos = group_by(nrl_ladders, season_no, season) %>%
  summarise(count = n(),
            sum_pos = sum(Pos)) %>%
...
error_sorted_pos = nrl_ladders %>%
  arrange(season_no, desc(Pts), desc(points_diff), desc(PF)) %>%
  mutate(sorted_row_number = row_number(),
         row_no_diff = row_number - sorted_row_number) %>%
  filter(!(row_no_diff == 0))

check_identical_pos = nrl_ladders %>%
  group_by(season_no, Pts, points_diff, PF) %>%
  summarise(count_seasons = n()) %>%
  filter(count_seasons > 1)

error_check_teams = nrl_ladders_name_join_fails
...
make_graph_nrl("ANN")   # Annandale
make_graph_nrl("BAL")   # Balmain Tigers
make_graph_nrl("CBY")   # Canterbury-Bankstown Bulldogs
#make_graph_nrl("CUM")    Cumberland - 1 season only
make_graph_nrl("SYD")   # Sydney Roosters
make_graph_nrl("GLE")   # Glebe Dirty Reds
make_graph_nrl("REB")   # Newcastle Rebels
make_graph_nrl("JET")   # Newtown Jets
make_graph_nrl("NOR")   # North Sydney Bears
make_graph_nrl("SOU")   # South Sydney Rabbitohs
make_graph_nrl("STG")   # St. George Dragons
make_graph_nrl("UNI")   # University
make_graph_nrl("WES")   # Western Suburbs Magpies
make_graph_nrl("MAN")   # Manly-Warringah Sea Eagles
make_graph_nrl("PAR")   # Parramatta Eels
make_graph_nrl("CRO")   # Cronulla-Sutherland Sharks
make_graph_nrl("PEN")   # Penrith Panthers
make_graph_nrl("CAN")   # Canberra Raiders
make_graph_nrl("ILA")   # Illawarra Steelers
make_graph_nrl("BRI")   # Brisbane Broncos
make_graph_nrl("GCC")   # Gold Coast Chargers
make_graph_nrl("NEW")   # Newcastle Knights
make_graph_nrl("NZL")   # New Zealand Warriors
make_graph_nrl("NQL")   # North Queensland Cowboys
make_graph_nrl("SQC")   # South Queensland Crushers
make_graph_nrl("PER")   # Perth Reds
make_graph_nrl("ADE")   # Adelaide Rams
#make_graph_nrl("HUN")    Hunter Mariners - 1 season only
make_graph_nrl("MEL")   # Melbourne Storm
make_graph_nrl("SGI")   # St. George-Illawarra Dragons
make_graph_nrl("WTI")   # Wests Tigers
make_graph_nrl("NTE")   # Northern Eagles
make_graph_nrl("GCT")   # Gold Coast Titans
make_graph_nrl("DOL")   # Dolphins


# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# export file to csv format
names(nrl_all_time_ladder) <- gsub(x = names(nrl_all_time_ladder), pattern = "_", replacement = " ") 

setwd(output_path)
save(tables, file = "nrl_ladders_raw.Rdata")
save(nrl_ladders, file = "nrl_ladders.Rdata")
write.csv(nrl_ladders, file = "nrl_ladders_full.csv")
write.csv(nrl_all_time_ladder, file = "nrl_all_time_ladder.csv")
setwd(path) 
...
# Change dotted line for no. of teams in finals - 7/5 in 1997, 10 in 1998

# Check count_gf; amend gf_years (incomplete - based on AFL)

# Check streaks for gaps - e.g. years when there was no Grand Final or Finals series
# longest_streaks_missed_finals should not include years before 1926 when no finals except play-offs were played

# Graph of season_totals data - ave_points_scored_team, min & max


# Future:
# Start on NRLW when there is enough data for meaningful charts
